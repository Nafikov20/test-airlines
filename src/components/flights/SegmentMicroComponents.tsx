...
import { TimeIcon } from '../time-icon/TimeIcon';
import './Flights.css';
...
export const Destination: React.FC<DestinationProps> = (props) => {
  return (
    <div className='f__segment__destination'>
...
export const Duration: React.FC<{ duration: number }> = (props) => {
  ...
  return (
    <div className='f__segment__departure_duration'>
...
export const Transfer: React.FC<{ transfer: number }> = (props) => {
  return <div className='f__segment__transfer'>{props.transfer === 1 ? <Orange v='1 пересадка' /> : <Line />}</div>;
};
